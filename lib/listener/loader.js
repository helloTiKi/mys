...
class ListenerLoader {
   ...
   */
  async load(client) {
    console.log(`ListenerLoader:${count++}`)
    this.client = client
    ...
    for (let File of files) {
      try {
        ...
        console.log(`listener:${listener.prefix + listener.event}`)
        if (lodash.isArray(listener.event)) {
          listener.event.forEach((type) => {
            ...
            this.client.wechat.on(listener.prefix + type, event => listener[e](event))
          })
        } else {
          ...
          this.client.wechat.on(listener.prefix + listener.event, event => listener[e](event))
        }
      } catch (e) {
