...
function getTimeInterval(hours = 0, minutes = 0, seconds = 0) {
  let now = new Date();
  let time = new Date()
...
class PluginsLoader {
  ...
  dealMsg(e) {
    ...
    if (e.user_id && cfg.masterQQ.includes(e.user_id)) {
      e.isMaster = true
    }
    ...
    this.task.forEach((val) => {
      ...
      val.job = schedule.scheduleJob(val.cron, async () => {
        try {
          ...
          redis.setEx(key, Math.floor(getTimeInterval(0, 1) / 1000) - 1, "1")
        } catch (error) {
  ...
  checkBlack(e) {
    ...
    /** 黑名单qq */
    if (other.blackQQ && other.blackQQ.includes(e.user_id)) {
      return false
    }
    ...
    if (e.group_id) {
      /** 白名单群 */
      if (other.whiteGroup) {
        if (other.whiteGroup.includes(e.group_id)) return true
        return false
      }
      /** 黑名单群 */
      if (other.blackGroup && other.blackGroup.includes(e.group_id)) {
        return false
      }
    }
