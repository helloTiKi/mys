...
class Puppeteer {
  ...
  async getHttpImage(name, data = {}) {
    async function isStart() {
      return axios.get('http://127.0.0.1:4567/', {})
        .then(e => {
          if (Frist) {
            console.log('图形渲染服务已启动')
          }
          Frist = true;
          return true
        })
        .catch(async e => {
          if (Frist) {
            Frist = false;
            console.log('正在启动图形渲染服务')
            cmd.exec('Start-Process', { shell: 'G:\\数据\\易语言\\易语言源码\\杂项\\原神辅助功能\\图片渲染.exe' })
            for (let i = 0; i < 3; i++) {
              await sleep(5000)
              if (await isStart()) {
    ...
    let ret = await axios.get('http://127.0.0.1:4567/getImage?url=', {
  ...
  async screenshot(name, data = {}) {
    if (this.isHttp) return await this.getHttpImage()
